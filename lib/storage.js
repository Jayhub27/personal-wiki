...
const adapters = {
  files: {
    ...
    async save(article) {
      const p = path.join(this.CONTENT_DIR, slugify(article.slug) + ".md");
      ...
      const frontmatter = [
        ...
        article.reference ? `reference: ${article.reference}` : null,
        article.referenceLabel ? `referenceLabel: ${article.referenceLabel}` : null,
        article.lead ? `lead: ${String(article.lead).replace(/"/g, '\\"')}` : null,
        "---",
      ].filter(Boolean).join("\n");
  ...
  supabase: {
    ...
    async init() {
      ...
      if (!url || !key) throw new Error("STORAGE_ADAPTER=supabase requires SUPABASE_URL and SUPABASE_ANON_KEY (or SERVICE_ROLE) env vars");
  ...
  vercel: {
    ...
    async save(article) {
      const { sql } = await import("@vercel/postgres");
      await sql`
        INSERT INTO articles (slug, title, content, parent, reference, referenceLabel, lead, tags, created_at, updated_at)
